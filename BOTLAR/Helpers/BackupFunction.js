...
module.exports = {
  ...
  rolKur(role, newRole) {
    roleBackup.findOne({ roleID: role }, async (err, data) => {
      ...
      const sayı = Math.floor(length / Bots.length);
      if (sayı < 1) sayı = 1;
      const channelPerm = data.channelOverwrites.filter(e => client.guilds.cache.get(ayarlar.guildID).channels.cache.get(e.id))
      for await (const perm of channelPerm) {
        const bott = Bots[1]
        const guild2 = bott.guilds.cache.get(ayarlar.guildID)
