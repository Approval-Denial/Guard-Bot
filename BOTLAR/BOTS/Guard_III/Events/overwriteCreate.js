...
class ChannelOverwriteCreate {
  ...
  async run(oldChannel, newChannel) {
    ...
    if (utils.get("channelGuard").value()) try {
      ...
      newChannel.guild.members.ban(entry.executor.id, { reason: "Kanal İzini Açtığı İçin Uzaklaştırıldı" }).catch(error => client.logger.error(`Etkinlik: ${module.exports.name} \nHata: ` + error + ``));
      let logs = client.channels.cache.get(utils.get("channelLog").value());
      const log = new Discord.MessageEmbed().setColor("RANDOM").setTitle(client.user.username + ' - Kanal İzin Açma Koruması').setDescription(`**${entry.executor.tag}** \`(${entry.executor.id})\` tarafından kanal güncellendi! Kanalı güncelleyen kişiyi sunucudan attım ve kanalı eski haline getirdim.\nGüncellenen Kanal: \`${oldChannel.name}\``).setTimestamp()
...
module.exports = ChannelOverwriteCreate