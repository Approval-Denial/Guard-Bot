...
class EmojiCreate {
  ...
  async run(emoji, message) {
    ...
    if (utils.get("serverGuard").value()) try {
      ...
      const reason = "Emoji Oluşturma Koruması"
      await emoji.delete({ reason: reason }).catch(error => client.logger.error(`Etkinlik: ${module.exports.name} \nHata: ` + error + ``));
      ...
      const log = new Discord.MessageEmbed().setColor("RANDOM").setTitle(client.user.username + ' - Emoji Koruması').setDescription(`**${entry.executor.tag}** \`(${entry.executor.id})\` tarafından emoji açıldı! Emojiyi açan kişiyi cezalıya attım ve emojiyi sildim.\nCeza Numarası: (\`#${ceza.id}\`)`).setTimestamp()
